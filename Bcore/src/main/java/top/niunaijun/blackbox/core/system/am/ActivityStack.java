...
public class ActivityStack {
    ...
    public int startActivityLocked(int userId, Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode, int flags, Bundle options) {
        ...
        // Activity aliases are manifest entries, not Java Activity classes.
        // Android's real ActivityTaskManager resolves them to targetActivity
        // before the client instantiates the component.  Since virtual
        // activities bypass that resolution, do it here (including alias
        // chains used by recent Instagram builds).
        for (int aliasDepth = 0; activityInfo != null && aliasDepth < 8; aliasDepth++) {
            String aliasTarget = activityInfo.targetActivity;
            // Some Android releases omit targetActivity when the parsed
            // ActivityInfo is marshalled through our package service. Recover
            // it from the source package's authoritative system metadata.
            if (aliasTarget == null) {
                try {
                    ActivityInfo systemInfo = BlackBoxCore.getPackageManager().getActivityInfo(
                            new ComponentName(activityInfo.packageName, activityInfo.name),
                            android.content.pm.PackageManager.MATCH_DISABLED_COMPONENTS);
                    aliasTarget = systemInfo.targetActivity;
            ...
            ComponentName targetComponent =
                    new ComponentName(activityInfo.packageName, aliasTarget);
            ActivityInfo targetInfo = BPackageManagerService.get()
                    .getActivityInfo(targetComponent, GET_ACTIVITIES, userId);
            if (targetInfo == null) {
                break;
            }

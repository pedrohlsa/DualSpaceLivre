...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    private ResolveInfo chooseBestActivity(Intent intent, String resolvedType,
        ...
        if (query != null) {
            ...
            // Packages such as Instagram expose several launcher aliases and
            // enable exactly one of them through the real PackageManager.  The
            // parser contains all aliases, including disabled ones, so simply
            // taking the first virtual match can try to instantiate an alias
            // name as a Java Activity class.  When the source package is still
            // installed on the device, mirror Android's currently selected
            // component and map it back to the virtual result.
            if (N > 0 && Intent.ACTION_MAIN.equals(intent.getAction())) {
                try {
                    ResolveInfo systemResolve =
                            BlackBoxCore.getPackageManager().resolveActivity(new Intent(intent), flags);
                    if (systemResolve != null && systemResolve.activityInfo != null) {
                        String systemPackage = systemResolve.activityInfo.packageName;
                        String systemName = systemResolve.activityInfo.name;
                        for (ResolveInfo candidate : query) {
                            if (candidate != null && candidate.activityInfo != null
                                    && Objects.equals(systemPackage, candidate.activityInfo.packageName)
                                    && Objects.equals(systemName, candidate.activityInfo.name)) {
                                return candidate;
                            }
                        }
                    }

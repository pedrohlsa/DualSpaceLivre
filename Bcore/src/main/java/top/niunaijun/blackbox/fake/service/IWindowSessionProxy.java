...
public class IWindowSessionProxy extends BinderInvocationStub {
    ...
    public static class AddToDisplayAsUser extends AddToDisplay {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            // IWindowSession.addToDisplayAsUser(..., viewVisibility,
            // displayId, userId, ...). UserHandle is virtualized to space 0
            // inside guest apps, but WindowManager validates against the real
            // Android profile that owns the host process.
            if (args != null && args.length > 4 && args[4] instanceof Integer) {
                args[4] = BlackBoxCore.getHostUserId();
            }

    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                ...
                // ActivityStack may already have resolved a launcher alias to
                // its real target Activity. Keep that exact target while the
                // virtual process is restarted; asking for the launch intent
                // again can reintroduce the alias and make ActivityThread try
                // to instantiate the alias name as a Java class.
                Intent launchIntentForPackage = stubRecord.mTarget == null
                        ? BlackBoxCore.getBPackageManager().getLaunchIntentForPackage(
                                activityInfo.packageName, stubRecord.mUserId)
                        : new Intent(stubRecord.mTarget);

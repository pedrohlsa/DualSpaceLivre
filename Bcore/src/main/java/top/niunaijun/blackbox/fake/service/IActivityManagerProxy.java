...
import top.niunaijun.blackbox.core.identity.VirtualAdvertisingIdService;
...
public class IActivityManagerProxy extends ClassInvocationStub {
    ...
    public static Object BindServiceCommon(Object who, Method method, Object[] args,int callingPackageIndex) throws Throwable {
        try {
            ...
            if (VirtualAdvertisingIdService.handles(intent) && connection != null) {
                ...
                VirtualAdvertisingIdService.connect(
                        connection.asBinder(),
                        advertisingBinder
                );
                Slog.d(TAG, "Bound virtual Advertising ID service for user " + userId);
                return 1;
            }
            ...
            if (resolveInfo != null) {
                ...
                args[callingPackageIndex] = BlackBoxCore.getHostPkg();
                // The proxy service is launched against the host process, which
                // may run under a non-zero Android user (e.g. user 11). The
                // guest still reports user 0, so rewrite the userId argument to
                // the real host user or AMS rejects the bind with
                // "asks to run as user 0 but is calling from uid u11...".
                replaceLastUserId(args);
    ...
    public static class UnbindService extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (VirtualAdvertisingIdService.disconnect(iServiceConnection.asBinder())) {
                Slog.d(TAG, "Unbound virtual Advertising ID service");
                return true;
            }
    ...
    public static class RegisterReceiverWithFeature extends MethodHook{
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            int userIdIndex = getPermissionIndex() + 1;
            if (userIdIndex < args.length && args[userIdIndex] instanceof Integer) {
                args[userIdIndex] = BlackBoxCore.getHostUserId();
            }
    ...
    public static class RegisterReceiver extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            int userIdIndex = permissionIndex + 1;
            if (userIdIndex < args.length && args[userIdIndex] instanceof Integer) {
                args[userIdIndex] = BlackBoxCore.getHostUserId();
            }
            return method.invoke(who, args);

...
public class ILocationManagerProxy extends BinderInvocationStub {
    ...
    public static class IsLocationEnabledForUser extends MethodHook {
        ...
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            // The guest virtualizes its own user to 0, but the host process may
            // run under a different Android user (e.g. user 11). Querying user 0
            // from a user-11 uid throws SecurityException (INTERACT_ACROSS_USERS)
            // and crashes apps like Instagram on resume. Rewrite the userId
            // argument to the real host user so the query stays same-user.
            replaceLastUserId(args);
            try {
                return method.invoke(who, args);
            } catch (Throwable e) {
                if (e.getCause() instanceof SecurityException || e instanceof SecurityException) {
                    Log.w(TAG, "isLocationEnabledForUser denied, returning true");
                    return true;
                }
                throw e;
            }
